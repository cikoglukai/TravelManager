<template>
  <div class="explore-page">
    ...
    <!-- ── Activity / community discovery (Insta-style row under the globe) ── -->
    <section class="discover" v-if="discoverItems.length || suggestedTravellers.length">
      <div class="discover-grid">
        <div class="discover-col">
          <h2>Recent activity</h2>
          <ul v-if="discoverItems.length" class="activity-list">
            <li v-for="i in discoverItems" :key="i.id" class="activity-item">
              <NuxtLink :to="`/users/${i.author}`" class="who">{{ i.author }}</NuxtLink>
              <span class="what"> · {{ i.verb || 'shared' }} </span>
              <NuxtLink v-if="i.objectId" :to="`/trips/${i.objectId}`" class="where">
                {{ i.destination || `trip #${i.objectId}` }}
              </NuxtLink>
            </li>
          </ul>
          <p v-else class="muted">Follow other travellers to fill your activity feed.</p>
        </div>

        <div class="discover-col">
          <h2>Travellers to follow</h2>
          <ul v-if="suggestedTravellers.length" class="suggest-list">
            <li v-for="s in suggestedTravellers" :key="s.uid" class="suggest-item">
              <NuxtLink :to="`/users/${s.uid}`" class="suggest-name">{{ s.name || s.uid }}</NuxtLink>
              <FollowButton :user-uid="s.uid" />
            </li>
          </ul>
...
// ── Activity + suggested-travellers data (loaded once on mount) ──
const discoverItems = ref([])
const suggestedTravellers = ref([])

async function loadDiscover() {
  // Feed (auth required — fail silently for guests)
  try {
    const items = await apiFetch('/api/feed').catch(() => [])
    discoverItems.value = (items || []).slice(0, 8)
  } catch { /* guests have empty feed */ }

  // Suggested travellers — top recent public trips → distinct authors
  try {
    const all = await apiFetch('/api/trips/all')
    const seen = new Set()
    const suggest = []
    for (const t of all || []) {
      if (!t.user_uid || seen.has(t.user_uid)) continue
      if (user.value?.firebase_uid === t.user_uid) continue
      seen.add(t.user_uid)
      suggest.push({ uid: t.user_uid, name: t.user_name })
      if (suggest.length >= 6) break
    }
    suggestedTravellers.value = suggest
  } catch { /* ignore */ }
}
...
/* ── Activity / suggested travellers ── */
.discover { max-width: 1100px; margin: 2rem auto 4rem; padding: 0 1rem; }
.discover h2 { font-size: 1.1rem; margin: 0 0 0.75rem; }
.discover-grid { display: grid; grid-template-columns: 1.4fr 1fr; gap: 1.25rem; }
@media (max-width: 720px) { .discover-grid { grid-template-columns: 1fr; } }
.activity-list, .suggest-list { list-style: none; padding: 0; margin: 0; display: grid; gap: 0.5rem; }
.activity-item, .suggest-item {
  padding: 0.75rem 1rem; background: white; border-radius: 10px;
  box-shadow: 0 2px 8px rgba(0,0,0,0.04);
}
.suggest-item { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
.who, .suggest-name, .where { color: var(--navy, #0f172a); font-weight: 600; text-decoration: none; }
.who:hover, .suggest-name:hover, .where:hover { text-decoration: underline; }
.what { color: #475569; }
.muted { color: #64748b; }
</style>
